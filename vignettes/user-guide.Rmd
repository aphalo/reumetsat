...
```{r}
vars_OMI_AURA_UV_he5(one.file.name)
```

By default only the boundaries of the grid are returned.

```{r}
grid_OMI_AURA_UV_he5(one.file.name)
```

With defaults all variables are read, and because the data can include multiple geographic grid points, `Longitude` and `Latitude` are always returned in the data frame.

```{r}
global_one_day.tb <- read_OMI_AURA_UV_he5(one.file.name)
...
```{r}
uvi_global.tb <-
  read_OMI_AURA_UV_he5(one.file.name,
                       vars.to.read = "UVindex")
dim(uvi_global.tb)
...
As for other functions in the package, `read_OMI_AURA_UV_he5()` accepts a `character` vector of file names to read and returns a single concatenated data frame.
