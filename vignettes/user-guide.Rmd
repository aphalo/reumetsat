...
```
To read variables based on name matching we can first retrieve the names of all variables, programatically select some of them and pass them to parameter `vars.to.read`.
...
summer_viikki_geo.tb <-
  read_AC_SAF_UV_txt(two.file.names, keep.QC = FALSE, add.geo = TRUE)
