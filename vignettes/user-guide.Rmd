...
Variable names are consistent between the data frames returned by `read_AC_SAFT_hdf5()` and `read_AC_SAFT_txt()`, but the position of the columns, can vary. *Use names rather than numeric positional indices to extract columns!*
...
The only difference to the case of reading a single file is in the length of the character vector containing file names. *The different files read in the same call to `read_AC_SAFT_hdf5()` should share identical grids and contain the variables to be read.* If this is not the case, `read_AC_SAFT_hdf5()` should be used to read them individually and later combined, which is a slower approach.
