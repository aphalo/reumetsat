...
In the case of two files we can find what variables are in only one file by passing `set.oper = "setdiff"` as argument.

```{r}
vars_AC_SAF_UV_txt(two.file.names, keep.QC = FALSE, set.oper = "setdiff")
```
Alternatively we can check the variables present in each file.
