...
In the case of time-series data one may want to read several files for the same location, for example for different time periods at the same location or a few different locations for the same time period. This can be achieved by passing to parameter `files` a vector of file names.

One consideration is that when reading multiple time-series files, we must ensure that the variables we intend to read are present in all the files. The presence of additional variables in only some files is not a problem as the files are read selectively, and the name and position of columns found from the header of each file. We can find out which variables are present in all the files we intend to read as shown below, relying on the default behavior of function `vars_AC_SAF_UV_txt()`.

```{r}
two.file.names <- rep(one.file.name, 2)
```

We can ignore the quality control variables as they are always included in the files.
 
```{r}
vars_AC_SAF_UV_txt(two.file.names, keep.QC = FALSE)
```
...
summer_viikki_geo.tb <-
  read_AC_SAF_UV_txt(two.file.names, keep.QC = FALSE)
...
As memory is pre-allocated the set of variables to be read is decided when or before the first file is read. If no argument is passed to `vars.to.read`, the default becomes the set of variables found in the first file read. On the contrary, if an argument is passed, it determines the set of variables to read. If any variable in this set is not found in a file, currently, an error is triggered.
