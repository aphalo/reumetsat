#' EUV gridded original files
#'
#' @description Import \strong{gridded} European UV "EUV" data released by
...
#'
#' @details Function `read_AC_SAF_EUV_nc()` can be used to read the data
#'   stored in a file, either in full or selected variables. Query functions
#'   `vars_AC_SAF_EUV_nc()`, `grid_AC_SAF_EUV_nc()` and
#'   `date_AC_SAF_EUV_nc()` extract the names of the variables, the range of
...
#'   `read_AC_SAF_EUV_nc()`, and grid sub-setting during file reading is
...
#'
#' @return Function `read_AC_SAF_EUV_nc()` returns a data frame with columns
...
#'   attributes. `vars_AC_SAF_EUV_nc()` returns a `character` vector of
#'   variable names, `grid_AC_SAF_EUV_nc()` returns a data frame with two
...
#'   `date_AC_SAF_EUV_nc()` returns a named vector of class `Date`, by default
...
read_AC_SAF_EUV_nc <-
  function(files,
    ...
    files <-
      check_files_accessible(files, name.pattern = "^acsaf_euv_v.*\\.nc$")
    ...
    dates <- date_AC_SAF_EUV_nc(files)
...
#' @rdname read_AC_SAF_EUV_nc
#'
#' @param set.oper character One of `"intersect"`, or `"union"`.
#'
#' @export
#'
vars_AC_SAF_EUV_nc <- function(files,
                                set.oper = "intersect") {

  files <- check_files_accessible(files, name.pattern = "^acsaf_euv_v.*\\.nc$")
...
#' @export
#'
grid_AC_SAF_EUV_nc <- function(files,
                                expand = FALSE) {
  ...
  expanded.tb <-
    read_AC_SAF_EUV_nc(files, vars.to.read = character(), verbose = FALSE)
...
#' @export
#'
date_AC_SAF_EUV_nc <- function(files,
  ...
  z <- as.Date(sub(".*_([0-9]{8})\\..*", "\\1", files), format = "%Y%m%d")
  if (use.names) {
    names(z) <- files
  }
  z
