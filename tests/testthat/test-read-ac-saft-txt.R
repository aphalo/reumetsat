test_that("reads one time series file", {
  ...
  expect_equal(vars_AC_SAF_txt(one.file.name), all.variables)
  expect_equal(vars_AC_SAF_txt(one.file.name, keep.QC = FALSE),
               no.qc.variables)

  test1.df <- read_AC_SAF_txt(one.file.name)
  ...
  test2.df <- read_AC_SAF_txt(one.file.name, add.geo = TRUE)
  ...
  test3.df <- read_AC_SAF_txt(one.file.name, keep.QC = FALSE)
  expect_equal(colnames(test3.df), no.qc.variables)
  expect_equal(nrow(test3.df), 153)
  expect_equal(sum(is.na(test3.df)), 8)

  test4.df <- read_AC_SAF_txt(one.file.name, add.geo = TRUE, keep.QC = FALSE)
  ...
  test5.df <- read_AC_SAF_txt(one.file.name, vars.to.read = vars.to.read)
  expect_equal(colnames(test5.df), c("Date", vars.to.read))
  expect_equal(nrow(test5.df), 153)
  expect_equal(sum(is.na(test5.df)), 4)

  test6.df <- read_AC_SAF_txt(one.file.name, vars.to.read = vars.to.read, add.geo = TRUE)
...
test_that("errors are triggered", {
  ...
  expect_error(read_AC_SAF_txt(c("missing-file1", one.file.name, "missing-file2"),
                                 verbose = FALSE))
  expect_error(read_AC_SAF_txt("missing-file", verbose = FALSE))

  # errors with file of wrong format
  expect_error(read_AC_SAF_txt("bad-file.txt", verbose = FALSE))
