test_that("reads one OMI/Aura whole grid HDF5 data file", {
  ...
  expect_equal(sUV_vars_OMUVBd_he5(one.file.name), all.variables)

  expect_equal(sUV_grid_OMUVBd_he5(one.file.name), grid.range)

  expanded.grid <- sUV_grid_OMUVBd_he5(one.file.name, expand = TRUE)
  ...
  expect_equal(names(sUV_date_OMUVBd_he5(one.file.name)), basename(one.file.name))
  expect_equal(unname(sUV_date_OMUVBd_he5(one.file.name)), as.Date("2024-10-01"))
  expect_equal(sUV_date_OMUVBd_he5(one.file.name, use.names = FALSE), as.Date("2024-10-01"))

  test1.df <- sUV_read_OMUVBd_he5(one.file.name, verbose = FALSE)
  ...
  test2.df <- sUV_read_OMUVBd_he5(one.file.name,
...
test_that("errors are triggered", {
  ...
  expect_error(sUV_read_OMUVBd_he5(c("missing-file1", one.file.name, "missing-file2"),
                                   data.product = "Surface UV",
                                   verbose = FALSE))
  expect_error(sUV_vars_OMUVBd_he5(c("missing-file1", one.file.name, "missing-file2")))
  expect_error(sUV_vars_OMUVBd_he5(one.file.name, set.oper = "bad.oper"))
  expect_error(sUV_grid_OMUVBd_he5(c("missing-file1", one.file.name, "missing-file2")))
  expect_error(sUV_date_OMUVBd_he5(c("missing-file1", one.file.name, "missing-file2")))

  expect_error(sUV_read_OMUVBd_he5("missing-file", verbose = FALSE))
  expect_error(sUV_read_OMUVBd_he5(c(one.file.name, "missing-file"), verbose = FALSE))
  expect_error(sUV_read_OMUVBd_he5(1L, verbose = FALSE))

  # verbose works
  expect_no_error(z <- sUV_read_OMUVBd_he5(one.file.name, verbose = TRUE))
