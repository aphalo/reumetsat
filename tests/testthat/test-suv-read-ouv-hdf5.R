test_that("reads one grid data file", {
  ...
  expect_equal(sUV_vars_OUV_hdf5(one.file.name), all.variables)
  expect_equal(sUV_vars_OUV_hdf5(one.file.name, keep.QC = FALSE),
               all.variables.no.QC)

  expect_equal(sUV_grid_OUV_hdf5(one.file.name), grid.range)

  expanded.grid <- sUV_grid_OUV_hdf5(one.file.name, expand = TRUE)
  ...
  expect_equal(names(sUV_date_OUV_hdf5(one.file.name)), basename(one.file.name))
  expect_equal(unname(sUV_date_OUV_hdf5(one.file.name)), as.Date("2024-06-21"))
  expect_equal(sUV_date_OUV_hdf5(one.file.name, use.names = FALSE), as.Date("2024-06-21"))

  test1.df <- sUV_read_OUV_hdf5(one.file.name, verbose = FALSE)
  ...
  test1a.df <-
    sUV_read_OUV_hdf5(one.file.name, keep.QC = FALSE, verbose = FALSE)
  ...
  test2.df <- sUV_read_OUV_hdf5(one.file.name,
...
test_that("reads two consistent grid data files", {
  ...
  expect_silent(sUV_vars_OUV_hdf5(two.file.names))
  expect_equal(sUV_vars_OUV_hdf5(two.file.names), all.variables)

  expect_silent(sUV_grid_OUV_hdf5(two.file.names))
  expect_equal(sUV_grid_OUV_hdf5(two.file.names), grid.range)

  expect_equal(names(sUV_date_OUV_hdf5(two.file.names)),
               basename(two.file.names))
  expect_equal(unname(sUV_date_OUV_hdf5(two.file.names)),
               as.Date(c("2024-06-21", "2024-06-22")))
  expect_equal(sUV_date_OUV_hdf5(two.file.names, use.names = FALSE),
               as.Date(c("2024-06-21", "2024-06-22")))

  test1.df <- sUV_read_OUV_hdf5(two.file.names, verbose = FALSE)
  expect_s3_class(test1.df, "data.frame", exact = TRUE)
  expect_s3_class(test1.df$Date, "Date", exact = TRUE)
  ...
  test2.df <- sUV_read_OUV_hdf5(two.file.names,
                                vars.to.read = vars.to.read, verbose = FALSE)
  expect_s3_class(test2.df, "data.frame", exact = TRUE)
  expect_s3_class(test2.df$Date, "Date", exact = TRUE)
...
test_that("reads two different grid data files", {
  ...
  expect_message(sUV_vars_OUV_hdf5(two.file.names))
  expect_setequal(sUV_vars_OUV_hdf5(two.file.names), shared.variables)
  expect_setequal(sUV_vars_OUV_hdf5(two.file.names, set.oper = "intersect"), shared.variables)
  expect_setequal(sUV_vars_OUV_hdf5(two.file.names, set.oper = "union"), all.variables)
  expect_error(sUV_vars_OUV_hdf5(two.file.names, set.oper = "setdiff"))
  ...
  test1.df <- sUV_read_OUV_hdf5(two.file.names, verbose = FALSE,
                                  vars.to.read = sUV_vars_OUV_hdf5(two.file.names))
  expect_s3_class(test1.df, "data.frame", exact = TRUE)
  expect_s3_class(test1.df$Date, "Date", exact = TRUE)
  expect_equal(length(unique(test1.df$Date)), 2)
...
test_that("errors are triggered", {
  ...
  expect_error(sUV_read_OUV_hdf5(c("missing-file1", one.file.name, "missing-file2"),
                                   data.product = "Surface UV",
                                   verbose = FALSE))
  expect_error(sUV_vars_OUV_hdf5(c("missing-file1", one.file.name, "missing-file2")))
  expect_error(sUV_grid_OUV_hdf5(c("missing-file1", one.file.name, "missing-file2")))
  expect_error(sUV_date_OUV_hdf5(c("missing-file1", one.file.name, "missing-file2")))

  expect_error(sUV_read_OUV_hdf5("missing-file", verbose = FALSE))
  expect_error(sUV_read_OUV_hdf5("O3MOUV_missing-file", verbose = FALSE))
  expect_error(sUV_read_OUV_hdf5(c(one.file.name, "missing-file"), verbose = FALSE))
  expect_error(sUV_read_OUV_hdf5(1L, verbose = FALSE))

  # fails early with wrong data product name
  expect_warning(sUV_read_OUV_hdf5(one.file.name,
                                     data.product = "bad-product-name",
                                     verbose = FALSE))
  # case insensitive
  expect_no_error(z <- sUV_read_OUV_hdf5(one.file.name,
                                           data.product = "surface uv",
                                           verbose = FALSE))
  ...
  # verbose works
  expect_no_error(z <- sUV_read_OUV_hdf5(one.file.name, verbose = TRUE))
